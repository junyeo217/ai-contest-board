...
function guidelineMarkup(item) {
  ...
  if (!guide) return `<details class="guide-detail"><summary>공모요강 보기 <span class="guide-label">확인 전</span></summary><div class="guide-unknown"><strong>${escapeHtml(config.guidelineBlockers?.[item.title] || '상세 공모요강은 아직 구조화하지 않았습니다.')}</strong><p>접수 기간: ${escapeHtml(item.submission_start || '미정')} ~ ${escapeHtml(item.submission_end || '미정')}</p><p>참가 자격, 작품 규격, 시상, 권리 조건은 원문에서 확인하세요.</p>${linkMarkup(item)}</div></details>`;
...
function pageHtml(page, data) {
  ...
  const appOptions = JSON.stringify({ fetchPath: page.fetchPath, regionLabel: page.regionLabel, musicVideoWhitelist: config.musicVideoWhitelist, guidelines: config.guidelines, guidelineBlockers: config.guidelineBlockers || {} }).replace(/</g, '\\u003c');
  return `<!doctype html>
<html lang="ko">
<head>
<meta charset="utf-8">
